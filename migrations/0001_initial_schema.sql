-- =====================================================================
-- Migration 0001: Initial schema for DO Manager
--
-- Full schema with 7 additions vs original schema.sql:
--   (a) key_version INTEGER DEFAULT 1 on api_credentials
--   (b) credential_id INTEGER REFERENCES api_credentials on job_queue
--   (c) root_password_encrypted, root_password_iv on servers
--   (d) network_json on servers
--   (e) last_probed_at on api_credentials
--   (f) idx_audit_log_created, idx_servers_external indexes
--   (g) job_queue.status comment includes 'cancelled'
-- =====================================================================
...
CREATE TABLE api_credentials (
  ...
  key_version            INTEGER DEFAULT 1,        -- tracks which ENCRYPTION_KEY was used (1=V1, 2=V2)
  ...
  last_probed_at         TEXT,                    -- when exhaust probe last checked this token
  last_used_at           TEXT,
  created_at             TEXT NOT NULL DEFAULT (datetime('now'))
);
...
CREATE TABLE servers (
  ...
  size_slug             TEXT,
  ip_address            TEXT,
  root_password_encrypted TEXT,            -- AES-GCM ciphertext of DO root password
  root_password_iv        TEXT,            -- IV for decryption
  network_json          TEXT,              -- JSON blob of DO API networks.v4/v6
...
-- ---------------------------------------------------------------------
-- Additional indexes for performance (additions f & g)
-- ---------------------------------------------------------------------
CREATE INDEX IF NOT EXISTS idx_audit_log_created ON audit_log (created_at);
CREATE INDEX IF NOT EXISTS idx_servers_external ON servers (external_id);
